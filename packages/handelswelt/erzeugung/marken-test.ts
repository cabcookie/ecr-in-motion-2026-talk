...
import { NICHT_ERLAUBT } from './decknamen';
...
for (const text of [
  ...
  const treffer = NICHT_ERLAUBT.filter((m) => new RegExp(m.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i').test(text[1]));
  pruefe(`${text[0]} ohne echte Marken`, treffer.length === 0, treffer.join(', '));
}
