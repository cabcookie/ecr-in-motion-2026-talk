/**
 * Outlook MCP Server – ALDI SUED Microsoft Outlook (stdio Transport)
 *
 ...
 *   Command: npx
 *   Arguments: tsx /path/to/quick/simulation/packages/mcp-outlook/src/server-stdio.ts
 */
...
main().catch((err) => {
  console.error("[MCP Outlook stdio] Fatal error:", err);
  process.exit(1);
});
