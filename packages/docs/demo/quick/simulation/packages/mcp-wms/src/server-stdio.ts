/**
 * WMS MCP Server – ALDI SUED Manhattan WMS (stdio Transport)
 *
 ...
 *   Command: npx
 *   Arguments: tsx /path/to/quick/simulation/packages/mcp-wms/src/server-stdio.ts
 */
...
main().catch((err) => {
  console.error("[MCP WMS stdio] Fatal error:", err);
  process.exit(1);
});
