...
function Note({
  ...
  return (
    <div className="grid grid-cols-[92px_1fr] items-start gap-4 border-b border-hair py-3 last:border-b-0">
      <dt
        className={`pt-0.5 font-mono text-[10px] tracking-[0.12em] uppercase ${tone}`}
      >
        {label}
      </dt>
...
export function OperatorView() {
  const { index, step, next, prev, goto, connected, transport } =
    useNavigation(TOTAL);
          ...
          <div className="flex min-w-0 flex-col">
            {panel?.say && (
              <div className="mb-5">
                ...
                <p className="m-0 text-[26px] leading-[1.45] text-pretty text-fg">
                  {panel.say}
                </p>
              </div>
            )}
            ...
            <div className="mt-auto flex flex-col gap-2 border-t border-hair pt-4">
              {BLOCKS.map((b) => (
                <div key={b.n} className="flex items-center gap-2">
                  <span
                    className="w-22 shrink-0 font-mono text-[10px] tracking-widest uppercase"
                      ...
                      return (
                        <button
                          ...
                          className={`h-7 w-9 rounded font-mono text-[11px] tabular-nums transition-colors ${
                            current
                              ? "text-stage"
                              : "border border-hair text-fg-3 hover:text-fg"
                          }`}
                          style={
                            current ? { background: ACCENT[b.n] } : undefined
                          }
                        >
